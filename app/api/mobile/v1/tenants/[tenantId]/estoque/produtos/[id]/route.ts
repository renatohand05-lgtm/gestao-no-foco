...
import { composeStockProductDetail } from "@/lib/mobile/stock-compose";
...
/** GET /api/mobile/v1/tenants/:tenantId/estoque/produtos/:id */
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const dto = await composeStockProductDetail({
    ...
    if (err instanceof Error && err.message === "NOT_FOUND") {
      return mobileNotFound("Produto não encontrado");
    }
