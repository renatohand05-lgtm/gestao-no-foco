...
import { composeOpsVehicleDetail } from "@/lib/mobile/operations-compose";
...
/** GET /api/mobile/v1/tenants/:tenantId/operacao/vehicles/:id */
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const dto = await composeOpsVehicleDetail({
    ...
    if (err instanceof Error && err.message === "NOT_FOUND") {
      return mobileNotFound("Veículo não encontrado");
    }
