import { fetchStockAlerts } from "@/api/mobile-api";
import {
  STOCK_VIEW_PERMS,
  StockSkeleton,
  stockErrorMessage,
...
export default function EstoqueAlertasScreen() {
  ...
  const query = useQuery({
    queryKey: qk.module(tenantId || null, branchId, "stock-alerts"),
    enabled: Boolean(tenantId) && online && canView,
    staleTime: 60_000,
    queryFn: async () => {
      const result = await fetchStockAlerts({ tenantId, branchId });
  ...
  if (!online) {
    return (
      <SafeAreaScreen edges={["left", "right"]}>
        <ErrorState
          title="Offline"
          message="Alertas detalhados exigem conexão. Resumo pode estar no Estoque Home offline."
        />
      </SafeAreaScreen>
    );
  }
    ...
    return (
      <SafeAreaScreen edges={["left", "right"]}>
        <ErrorState
          ...
          message={stockErrorMessage(query.error, "Não foi possível carregar alertas.")}
          action={
