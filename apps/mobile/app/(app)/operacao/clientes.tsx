import { fetchOpsCustomers } from "@/api/mobile-api";
import {
  OPS_VIEW_PERMS,
  OpsSkeleton,
  opsErrorMessage,
...
export default function OperacaoClientesScreen() {
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      const result = await fetchOpsCustomers({
        tenantId,
        branchId,
  ...
  if (!online) {
    return (
      <SafeAreaScreen edges={["left", "right"]}>
        <ErrorState title="Offline" message="Lista de clientes exige conexão." />
      </SafeAreaScreen>
    );
  }
    ...
    <SafeAreaScreen edges={["left", "right"]}>
      ...
      {query.isLoading && !query.data ? (
        ...
        <ErrorState
          ...
          message={opsErrorMessage(query.error, "Não foi possível listar clientes.")}
          action={
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              onPress={() => router.push(`/operacao/clientes/${item.id}`)}
            ...
            >
              <Card style={{ marginHorizontal: 16, marginBottom: 8 }}>
                ...
                >
                  {[item.telefone, item.cidade].filter(Boolean).join(" · ") || "—"}
                </Text>
              </Card>
            </Pressable>
          )}
