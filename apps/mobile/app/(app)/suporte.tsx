...
export default function SuporteScreen() {
  ...
  useEffect(() => {
    let ignore = false;
    (async () => {
      const result = await fetchSupportConversation(tenantId);
      if (ignore) return;
      if (result.ok) {
  ...
  async function handleSend() {
    ...
    setDraft("");
    setSending(true);
    const result = await sendSupportMessage(tenantId, body);
    setSending(false);
    if (result.ok) {
      setMessages(result.data.messages);
    } else {
            ...
            messages.map((m) => (
              ...
              >
                <Text
                  variant="body"
                  style={{
                    color: m.senderRole === "tenant_user" ? "#0B0F14" : colors.text,
                  }}
                >
                  {m.body}
                </Text>
                <Text
                  ...
                  style={{
                    marginTop: 4,
                    color: m.senderRole === "tenant_user" ? "#0B0F14AA" : colors.textMuted,
                  }}
                >
