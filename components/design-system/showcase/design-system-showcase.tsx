...
import { gofFocusRing } from "@/lib/design-system/primitives";
import {
  LEGACY_AUDIT,
  OFFICIAL_COMPONENTS,
  PRODUCT_BLOCKS,
...
/**
 * Showcase oficial — sem dados de negócio (Gate 19.5).
 */
    ...
    >
      ...
      >
        ...
        <p className={gofTypography.caption}>
          Preview max-width: {width}px · tema light (dark desativado na arquitetura) ·
          preferências não são persistidas.
        </p>
      ...
      >
        ...
        {/* Foundations / Typography / Colors / Spacing / Radius / Shadows / Motion */}
        <SectionShell
          id="foundations"
          title="Foundations"
        ...
        >
          <p className={gofTypography.subtitle}>
            Prefixo oficial: <code className="font-mono text-xs">gof*</code>. Camadas
            legado: <code className="font-mono text-xs">ex*</code>,{" "}
            <code className="font-mono text-xs">ds*</code>.
          </p>
        </SectionShell>

        <SectionShell id="typography" title="Typography" description="Escala tipográfica gof.">
        ...
        <SectionShell id="spacing" title="Spacing" description="Escala gofSpacing.">
          <div className="space-y-2">
            {Object.entries(gofSpacing).map(([k, v]) => (
              <div key={k} className="flex items-center gap-3">
                <span className={cn("w-10", gofTypography.caption)}>{k}</span>
                <div className={cn("h-3 bg-[var(--brand-gold)]/40", v.replace("gap-", "w-"))} />
        ...
        <SectionShell id="motion" title="Motion" description="gofMotion · máx. 250ms.">
          <div className="flex flex-wrap gap-3">
            {Object.keys(gofMotion).map((k) => (
              <span
                key={k}
                className={cn(
                  "border border-border/60 px-3 py-2 text-xs",
                  gofRadius.md,
                  gofMotion[k as keyof typeof gofMotion],
                )}
              >
                {k}
              </span>
        ...
        <SectionShell id="icons" title="Icons" description="Lucide via DsIcon.">
          <p className={gofTypography.subtitle}>
            Ícones apenas Lucide. Tamanhos via <code className="font-mono text-xs">DsIcon</code>.
          </p>
        ...
        <SectionShell id="buttons" title="Buttons" description="Estados default/loading/disabled.">
          <div className="flex flex-wrap gap-2">
        ...
        <SectionShell id="inputs" title="Inputs" description="gofControl / Input.">
          <div className="max-w-sm space-y-2">
            <Label htmlFor="ds-input-demo">E-mail</Label>
            <Input id="ds-input-demo" placeholder="voce@empresa.com" className="h-11" />
            <Input disabled placeholder="Disabled" className="h-11" />
          </div>
        </SectionShell>

        <SectionShell id="forms" title="Form Controls" description="Labels + focus ring Brand.">
          <p className={gofTypography.caption}>
            Focus ring: <code className="font-mono">gofFocusRing</code> (ouro Brand).
          </p>
        </SectionShell>
          ...
          <div className="flex flex-wrap gap-2">
            {(
              ...
              (["soft", "solid", "outline"] as const).map((variant) => (
                <ExecutiveBadge key={`${tone}-${variant}`} tone={tone} variant={variant}>
                  {tone}/{variant}
                </ExecutiveBadge>
              )),
            )}
        ...
        <SectionShell id="kpis" title="KPIs" description="MetricCard com fixtures locais.">
          <div className="grid gap-3 sm:grid-cols-3">
            <MetricCard label="Exemplo" value="—" hint="Fixture local" tone="neutral" />
            <MetricCard label="Sucesso" value="100%" tone="success" />
            <MetricCard label="Alerta" value="72%" tone="warning" emphasize />
          </div>
        </SectionShell>
        ...
        <SectionShell id="tables" title="Tables" description="Empty / rows fixtures.">
          <ExecutiveTable
            columns={[
        ...
        <SectionShell id="navigation" title="Navigation" description="Links de categoria (esta página).">
          <p className={gofTypography.subtitle}>
            Navegação do showcase via âncoras. Sidebar/App nav do produto não incluem esta
            rota para membros comuns.
          </p>
        </SectionShell>

        <SectionShell id="header" title="Header" description="ExecutiveHeader + AppHeader Brand.">
          <ExecutiveHeader
        ...
        <SectionShell id="sidebar" title="Sidebar" description="AppSidebar Brand.">
          <p className={gofTypography.subtitle}>
            Sidebar real no AppShell. Showcase não remonta o shell completo (performance).
          </p>
        </SectionShell>
          ...
          <Sheet>
            ...
            <SheetContent>
              <SheetHeader>
                ...
                <SheetDescription>Preview técnico sem persistência.</SheetDescription>
              </SheetHeader>
            </SheetContent>
          </Sheet>
        </SectionShell>

        <SectionShell id="loading" title="Loading" description="BrandSplash + ExecutiveLoading.">
          <BrandSplash
        ...
        <SectionShell id="skeletons" title="Skeletons" description="ExecutiveSkeleton*.">
          <div className="grid gap-3 sm:grid-cols-2">
        ...
        <SectionShell id="empty" title="Empty States" description="ExecutiveEmptyState.">
          <ExecutiveEmptyState
            icon={Inbox}
            title="Sem registros"
        ...
        <SectionShell id="errors" title="Error States" description="Padrão de erro apresentacional.">
          <ExecutiveEmptyState
            icon={AlertCircle}
        ...
        <SectionShell id="charts" title="Charts" description="Sem gráficos pesados no showcase.">
          <p className={gofTypography.subtitle}>
            Charts de produto vivem no Dashboard. Aqui documentamos a regra: não montar
            séries reais nem libs extras nesta página.
          </p>
        ...
        >
          ...
          {PRODUCT_BLOCKS.filter((c) => c.category === "executive-blocks").map((c) => (
            <DocCard key={c.name} item={c} />
          ))}
        </SectionShell>

        <SectionShell id="onboarding" title="Onboarding" description="Cards e checklist.">
          {PRODUCT_BLOCKS.filter((c) => c.category === "onboarding").map((c) => (
            <DocCard key={c.name} item={c} />
          ))}
          <ExecutiveSection title="Regra" description="Máx. 4 passos · Pular sempre · sem bloquear." panel>
            <p className={gofTypography.caption}>
        ...
        <SectionShell id="legacy" title="Legacy Audit" description="Mapa de migração.">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[40rem] text-left text-xs" data-legacy-audit="">
              <thead>
