...
import { createChannelProvider, createWhatsAppProviderForTenant } from "./providers/factory.ts";
import {
  effectiveEmailMode,
...
export function shouldDispatchReal(input: {
  ...
  hasConnectedEvolutionChannel?: boolean;
}): boolean {
  ...
  if (input.channel === "whatsapp") {
    return (
      effectiveWhatsAppMode(env) === "meta_cloud" ||
      Boolean(input.hasConnectedEvolutionChannel)
    );
  }
...
export async function sendViaChannelProvider(input: {
  ...
  const evolutionChannel =
    input.channel === "whatsapp"
      ? await (await import("./providers/whatsapp-evolution.ts")).getTenantWhatsAppChannel(
          input.tenantId,
        )
      : null;
  const hasConnectedEvolutionChannel = evolutionChannel?.status === "connected";
  const providerName =
    input.channel === "email"
      ? "resend"
      : hasConnectedEvolutionChannel
        ? "evolution"
        : "meta_cloud";

  if (!shouldDispatchReal({ channel: input.channel, to: input.to, env, hasConnectedEvolutionChannel })) {
  ...
  const effectiveProvider =
    input.channel === "whatsapp" && hasConnectedEvolutionChannel
      ? await createWhatsAppProviderForTenant(input.tenantId, env)
      : provider;
  const result = await effectiveProvider.send({
    to: input.to,
    body: input.body,
    tenantId: input.tenantId,
  });
