...
/**
 * Sprint 22.5.1 / 22.10.1 — Server actions do assistente de importação de Vendas.
 *
 * Usa a MESMA Import Engine do Financeiro (parsers, segurança, mapeamento,
 * classificação, preview, histórico). Histórico/mapeamento → Supabase.
 * O commit NÃO cria vendas reais — staging EXPLÍCITO em memória até ligação
 * aos services de Vendas (lib/vendas).
 */
...
import { SALES_IMPORT_ADAPTER } from "./adapter.ts";
...
const MODULE_ID = SALES_IMPORT_ADAPTER.id;
const MODULE_KEY = SALES_IMPORT_ADAPTER.moduleKey;

function toError(error: unknown): { success: false; error: string } {
  return {
    success: false,
    error: error instanceof Error ? error.message : "Erro na importação de vendas.",
  };
}

export async function listSalesImportHistory(tenantSlug: string) {
  try {
...
export async function previewSalesImport(tenantSlug: string, formData: FormData) {
  try {
    ...
    const parsed = engine.parseFile({ fileName: file.name, mimeType: file.type, bytes });

    const preview = await engine.buildPreview({
      tenantId: tenant.id,
      module: MODULE_KEY,
      targetEntity: SALES_IMPORT_ADAPTER.targetEntity,
      parsed,
      targetFields: SALES_IMPORT_ADAPTER.fields,
    });
...
export async function updateSalesImportMapping(
  tenantSlug: string,
  sessionId: string,
...
export async function buildSalesImportReview(tenantSlug: string, sessionId: string) {
  try {
    const { tenant, engine } = await resolveModuleImportRuntime(
      tenantSlug,
      SALES_IMPORT_ADAPTER,
    ...
    const normalized = engine.normalize(
      session.parsed,
      session.mapping,
      SALES_IMPORT_ADAPTER.fields,
    );
    const review: ImportReviewRow[] = engine.buildReview(normalized, {
      domain: SALES_IMPORT_ADAPTER.classificationDomain,
    });
...
export async function patchSalesImportReviewRow(
  tenantSlug: string,
  sessionId: string,
  rowNumber: number,
  patch: {
  ...
  try {
    const { tenant } = await resolveModuleImportRuntime(tenantSlug, SALES_IMPORT_ADAPTER);
    const session = getImportWizardSession(MODULE_ID, sessionId, tenant.id);
    if (!session?.review) throw new Error("Revisão não encontrada. Gere a pré-visualização novamente.");
...
export async function commitSalesImport(
  ...
  try {
    ...
    revalidatePath(`/${tenantSlug}/integracoes/importar/vendas`);
    ...
    return {
      ...
      message:
        ...
        "Nenhuma venda real foi criada nesta etapa.",
    };
  } catch (error) {
    return toError(error);
  }
