...
export async function authorizeCrmRoute(
  request: Request,
  tenantId: string,
): Promise<{ ok: true; ctx: CrmRouteAuth } | { ok: false; response: NextResponse }> {
...
export function crmForbiddenOrError(err: unknown): NextResponse {
  if (err instanceof Error && err.message === "FORBIDDEN_CRM") {
    return mobileForbidden("Sem permissão de CRM neste tenant");
  }
