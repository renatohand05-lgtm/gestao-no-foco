...
export async function authorizeFinanceRoute(
  request: Request,
  tenantId: string,
): Promise<{ ok: true; ctx: FinanceRouteAuth } | { ok: false; response: NextResponse }> {
...
export function financeForbiddenOrError(err: unknown): NextResponse {
  if (err instanceof Error && err.message === "FORBIDDEN_FINANCE") {
    return mobileForbidden("Sem permissão financeira neste tenant");
  }
  return mobileError(mapDatabaseErrorToUserMessage(err));
}

export function financeJson<T>(data: T) {
