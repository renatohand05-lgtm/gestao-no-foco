...
export async function authorizeStockRoute(
  request: Request,
  tenantId: string,
): Promise<{ ok: true; ctx: StockRouteAuth } | { ok: false; response: NextResponse }> {
...
export function stockForbiddenOrError(err: unknown): NextResponse {
  if (err instanceof Error && err.message === "FORBIDDEN_STOCK") {
    return mobileForbidden("Sem permissão de estoque/compras neste tenant");
  }
