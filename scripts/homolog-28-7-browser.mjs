#!/usr/bin/env node
/**
 * Sprint 28.7 — Homologação browser autenticada + screenshots.
...
const OUT = resolve("docs/testing/evidence/28-7");
...
try {
  ...
  await gotoCheck(page, "crm/pipeline", "crm", "pipeline-desktop-dark");
  await gotoCheck(page, "clientes/funil", "crm", "kanban-desktop-dark");
  ...
  await gotoCheck(page, "compras/cotacoes", "purchases", "cotacoes-desktop-dark");
  await gotoCheck(
    page,
    "compras/indicadores",
    "purchases",
    "indicadores-desktop-dark",
  );
  ...
  await gotoCheck(
    ...
    "ordens/templates",
    "work-orders",
    "templates-desktop-dark",
  );

  // Agenda
  await gotoCheck(page, "agenda", "schedule", "semana-desktop-dark");
  await gotoCheck(page, "crm/agenda", "schedule", "crm-agenda-desktop-dark");
  ...
  // Regressão
  await gotoCheck(page, "dashboard", "regression", "dashboard-desktop-dark");
  ...
  await gotoCheck(page, "produtos", "regression", "produtos-desktop-dark");
  await gotoCheck(page, "analytics", "regression", "analytics-desktop-dark");
  await gotoCheck(
    page,
    "inteligencia",
    "regression",
    "inteligencia-desktop-dark",
  );
  await gotoCheck(page, "tributario", "regression", "tributario-desktop-dark");

  // Light + viewports sample
  ...
  console.log(
    `\nBrowser: ${report.summary.pass} PASS · ${report.summary.fail} FAIL · ${report.summary.shots} shots\n`,
  );
